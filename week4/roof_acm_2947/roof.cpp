//main idea: sort on end-point of segment in order to do a sweep line (vertical line contains idx of segments that it cut) from left to right and calculate where water from any segment will fall to
...
public:
    ...
    bool operator<(const EndPoint &o) const
    {
        ...
        else if(left == true) return (y < o.y); //if start of a segment, add (to sweepline) from bottom to top (to calculate dropTO segment)
        else return (y > o.y); //remove (from sweepline) from top to bottom (to calculate dropTO segment)
    }
};
...
vector<int> sky;
vector<int> col;
vector<int> dropTO;//idx of segment that water from cur idx segment will fall to
vector<int> nbSources;//nb of sources water will fall to idx segment
...
void readIn()
{
  ...
  col.assign(n, 0);
  dropTO.assign(n, -1);//-1 is the ground
...
void sweepping()
{
    REP(i, 2*n)
    {
        ...
        if(cur.left)//start of a segment add this segment to sweepline
        {
            ...
            if(EPs[2*cur.idx].y < EPs[2*cur.idx + 1].y)//if this is the lower end point find where water drops to
            {
        ...
        else//end of a segment -> remove from sweepline
        {
            ...
            if(EPs[2*cur.idx + 1].y < EPs[2*cur.idx].y)//if this is a lower end, find where water drops to
            {
                int dropIdx = findDrop(cur.x, cur.y);
                dropTO[cur.idx] = dropIdx;
            }
        }
        if(!sweepline.empty())//at a sweepline position (an end point of a segment) find the amount of water fall on the top
        {
